...
{
    $option_name = $key;
    $value = \Illuminate\Support\Facades\Cache::remember($option_name,6400, function () use($option_name) {
        try {
...
function single_post_share($url, $title, $img_url)
{
    ...
    $output .= '<li><a class="facebook" href="' . $facebook_share_link . '" aria-label="Share ' . htmlspecialchars($title) . ' on Facebook"><i class="fab fa-facebook-f"></i></a></li>';
    $output .= '<li><a class="twitter" href="' . $twitter_share_link . '" aria-label="Share ' . htmlspecialchars($title) . ' on Twitter"><i class="fab fa-twitter"></i></a></li>';
    $output .= '<li><a class="linkedin" href="' . $linkedin_share_link . '" aria-label="Share ' . htmlspecialchars($title) . ' on LinkedIn"><i class="fab fa-linkedin-in"></i></a></li>';
    $output .= '<li><a class="pinterest" href="' . $pinterest_share_link . '" aria-label="Share ' . htmlspecialchars($title) . ' on Pinterest"><i class="fab fa-pinterest-p"></i></a></li>';
...
function render_drag_drop_form_builder_field_markup($key, $type, $name, $placeholder, $required, $args = [])
{
    ...
    if ($type == 'file') {
        ...
        $output .= '<option value="mimes:doc,docx,txt,pdf,jpg,jpeg,png,webp"';
        if (isset($args['mimes_type']) && $args['mimes_type'] == 'mimes:doc,docx,txt,pdf,jpg,jpeg,png,webp') {
            $output .= 'selected="selected"';
        }
        $output .= '>' . __('mimes:doc,docx,txt,pdf,jpg,jpeg,png,webp') . '</option>';
        $output .= '</select></div>';
    }
    $output .= '</div></div></li>';
...
function createSlug($name, $language)
{
    // For Latin characters, use Laravel's Str::slug
    if (preg_match('/^[\p{Latin}\s0-9\-_]+$/u', $name)) {
        return Str::slug($name);
    }

    // For non-Latin characters (Japanese, Chinese, etc.)
    // Create a unique slug based on the original string
    $slug = preg_replace('/\s+/', '-', $name); // Replace spaces with hyphens
    $slug = preg_replace('/[^\p{L}\p{N}\-_]+/u', '', $slug); // Remove any other invalid characters

    // If slug is empty after processing (rare case), generate a random one
    if (empty($slug)) {
        $slug = 'item-' . strtolower(Str::random(5));
    }
...
function render_payment_gateway_for_form($cash_on_delivery = false)
{
    ...
    $all_gateway = [
        ...
        'payfast','cashfree','instamojo','marcadopago','squareup','cinetpay','paytabs','billplz','zitopay', 'toyyibpay', 'pagalipay','authorizenet','senangpay','iyzipay', 'awdpay'
    ];
    $output .= '<ul>';
